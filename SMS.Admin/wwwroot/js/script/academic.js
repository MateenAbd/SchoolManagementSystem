...
(function () {
    ...
    $(function () {
        ...
        // ADD: load courses for name-resolution (unfiltered)
        function loadCoursesForOptions2(cb) {
            $.ajax({
                url: "/Academic/GetCourseList",
                type: "GET",
                dataType: "json",
                ...
                .done(list => {
                    coursesCache2 = Array.isArray(list) ? list : [];
                })
                .always(() => cb && cb());
        }

        // ADD: derive a friendly course name (Class - Subject [AY])
        function courseNameById(id) {
            const c = coursesCache2.find(x => x.courseId === id);
            if (!c) return id;
            const subj = subjectsCache2.find(s => s.subjectId === c.subjectId)?.subjectName;
            const parts = [];
            if (c.className) parts.push(c.className);
            if (subj) parts.push(subj);
            const name = parts.join(" - ");
            return c.academicYear ? `${name} [${c.academicYear}]` : (name || id);
        }

        // Build #ttCourseId options from coursesCache2, filtered by AY and Class
        function rebuildTTCourseOptions() {
            const ay = $("#ttAY").val()?.trim();
            const cls = $("#ttClass").val()?.trim();
            const prev = $("#ttCourseId").val() || "";

            const list = (coursesCache2 || []).filter(c =>
                (ay ? c.academicYear === ay : true) &&
                (cls ? (c.className || "").toLowerCase() === cls.toLowerCase() : true) &&
                (c.isActive !== false)
            );

            const makeLabel = (c) => {
                const subj = subjectsCache2.find(s => s.subjectId === c.subjectId);
                const subjTxt = subj
                    ? `${(subj.subjectCode || "").trim()} ${(subj.subjectName || "").trim()}`.trim()
                    : `Subject #${c.subjectId}`;
                const classTxt = c.className || "";
                const ayTxt = c.academicYear ? ` [${c.academicYear}]` : "";
                return `${classTxt} - ${subjTxt}${ayTxt} (ID ${c.courseId})`;
            };

            const opts = ['<option value="">Select</option>']
                .concat(list.map(c => `<option value="${c.courseId}">${makeLabel(c)}</option>`));

            $("#ttCourseId").html(opts.join(""));
            if (prev) $("#ttCourseId").val(prev); // keep selection if still present
        }
        ...
        $("#ttAY, #ttClass").on("input change", rebuildTTCourseOptions);
        ...
        $("#btnValidateTimetable").on("click", function () {
            ...
            if (!dto.academicYear || !dto.className || !dto.courseId) {
                $("#ttValidateMsg").html(`<div class="alert alert-warning">Academic Year, Class and Course are required.</div>`);
                return;
            }
            $.ajax({
        ...
        $("#btnSaveTimetable").on("click", function () {
            ...
            const dto = buildTimetableDto();
            if (!dto.academicYear || !dto.className || !dto.courseId) {
                showAlert("#ttMsg", "warning", "Academic Year, Class and Course are required.");
                return;
            }
        ...
        function renderClassTimetable(list) {
          ...
          <td>${x.periodNo ?? ""}</td>
          <td>${time}</td>
          <td>${courseNameById(x.courseId)}</td>
      ...
      `).join("");
        ...
        loadSubjectsForOptions2(rebuildTTCourseOptions);
        loadRoomsForOptions2();
        loadCoursesForOptions2(rebuildTTCourseOptions);
